...
->
  # première instruction obligatoire, configure le vagrantfile, le 2 représente quelle version de vagrant
  Vagrant.configure("2") do |config|
    # écrire le nom de la machine au yeux de Vagrant
    config.vm.define "jbuanS" do
    # monter la box (image)
    config.vm.box = "generic/alpine319"
    # Prend auto la derniere version, cette ligne n'est pas obligé
    ...
    config.vm.define "jbuanS" do |control|
      ...
      control.vm.provider "virtualbox" do |v|
        # Configure nom de la VM
        v.name = "jbuanS"
        # La mémoire qu'on lui attribue
        v.memory = "1024"
        # le ou les coeurs
        v.cpus = "1"
        # ligne qui active l'option cable branché, qui aide pour la connectivité entre vm sur même réseau
